...
# تحديث decorator الـ login_required
...
# جدولة التحقق من التحديثات كل يوم
...
def owner_dashboard():
    ...
    return render_template_string(OWNER_DASHBOARD_TEMPLATE, **locals())
...
def error_logs():
    ...
    return jsonify([dict(error) for error in errors])

# 402. سجل مدفوعات
@app.route('/owner/payment-logs')
@owner_required
def payment_logs():
    """سجل المدفوعات"""
    conn = get_db()
...
# 403. سجل اشتراكات
@app.route('/owner/subscription-logs')
@owner_required
...
# 406. تقارير إدارية
@app.route('/owner/reports/<report_type>')
@owner_required
def admin_reports(report_type):
    """تقارير إدارية"""
    if report_type == 'users':
        return generate_users_report()
    elif report_type == 'messages':
        return generate_messages_report()
...
# 409. إدارة API
@app.route('/owner/api-keys')
@owner_required
def manage_api_keys_owner():
    """إدارة مفاتيح API"""
    conn = get_db()
    c = conn.cursor()
    c.execute('''
...
# 410. إدارة البوتات
@app.route('/owner/bots/settings', methods=['POST'])
@owner_required
def configure_bots():
    """تكوين البوتات"""
    settings = request.json
    ...
    c.execute('''
        UPDATE bot_settings 
        SET telegram_token=?, telegram_enabled=?, whatsapp_enabled=?
        WHERE id=1
    ''', (
        settings.get('telegram_token'),
        settings.get('telegram_enabled', 0),
        settings.get('whatsapp_enabled', 0)
    ))
    conn.commit()
    conn.close()
...
# 411. إدارة الإعلانات
@app.route('/owner/ads', methods=['GET', 'POST'])
@owner_required
def manage_ads():
    """إدارة الإعلانات"""
    if request.method == 'POST':
        ...
        c = conn.cursor()
        c.execute('''
            INSERT INTO ads (name, code, position, status)
            VALUES (?, ?, ?, ?)
        ''', (
            ad_data.get('name'),
            ad_data.get('code'),
            ad_data.get('position'),
...
# 412. إدارة العروض
@app.route('/owner/promotions', methods=['GET', 'POST'])
@owner_required
def manage_promotions():
    ...
    if request.method == 'POST':
        ...
        c = conn.cursor()
        c.execute('''
            INSERT INTO promotions (name, code, discount_type, discount_value, valid_from, valid_to)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            promo_data.get('name'),
            promo_data.get('code'),
            promo_data.get('discount_type'),
            promo_data.get('discount_value'),
            promo_data.get('valid_from'),
            promo_data.get('valid_to')
        ))
        conn.commit()
        conn.close()
    ...
    return jsonify([dict(promo) for promo in promotions])

# 413. إدارة الخصومات
@app.route('/owner/discounts', methods=['POST'])
@owner_required
def apply_discount():
    """تطبيق خصم"""
    ...
    c = conn.cursor()
    c.execute('''
        INSERT INTO user_discounts (user_id, discount_percent, reason, applied_by)
        VALUES (?, ?, ?, ?)
    ''', (user_id, discount_percent, reason, session['user_id']))
    conn.commit()
    conn.close()
...
# 414. إدارة الكوبونات
@app.route('/owner/coupons/generate', methods=['POST'])
@owner_required
def generate_coupons():
    """توليد كوبونات"""
    count = request.json.get('count', 10)
    value = request.json.get('value', 10)
    expiry_days = request.json.get('expiry_days', 30)
...
# 415. إدارة المحتوى
@app.route('/owner/content', methods=['GET', 'POST'])
@owner_required
def manage_content():
    """إدارة المحتوى"""
    if request.method == 'POST':
        ...
        c = conn.cursor()
        c.execute('''
            INSERT OR REPLACE INTO content (page, content_ar, content_en)
            VALUES (?, ?, ?)
        ''', (
            content_data.get('page'),
            content_data.get('content_ar'),
    ...
    return jsonify([dict(item) for item in content])

# 416. إدارة الأخبار
@app.route('/owner/news', methods=['GET', 'POST'])
@owner_required
def manage_news():
    """إدارة الأخبار"""
    if request.method == 'POST':
        ...
        c = conn.cursor()
        c.execute('''
            INSERT INTO news (title_ar, title_en, content_ar, content_en, image)
            VALUES (?, ?, ?, ?, ?)
        ''', (
            news_data.get('title_ar'),
            news_data.get('title_en'),
            news_data.get('content_ar'),
            news_data.get('content_en'),
            news_data.get('image')
        ))
        conn.commit()
        conn.close()
...
# 417. إدارة الإشعارات
@app.route('/owner/notifications/send', methods=['POST'])
@owner_required
def send_bulk_notification():
    """إرسال إشعار جماعي"""
    notification_data = request.json
    
    target = notification_data.get('target')  # all, admins, users, vip
    ...
    if target == 'all':
        ...
        c.execute("SELECT id FROM users WHERE role IN ('owner', 'admin')")
    elif target == 'vip':
        c.execute("SELECT id FROM users WHERE role='vip'")
    else:
    ...
    for user in users:
        c.execute('''
            INSERT INTO notifications (user_id, message_ar, message_en, type)
            VALUES (?, ?, ?, 'broadcast')
        ''', (user['id'], message_ar, message_en))
...
# 420. إدارة التحديثات
@app.route('/owner/updates', methods=['POST'])
@owner_required
def perform_update():
    """تطبيق تحديث"""
    update_version = request.json.get('version')
    
    # هنا هتكتب logic التحديث الفعلي
    # تحميل الملفات، تحديث قاعدة البيانات، إلخ
    
    conn = get_db()
    c = conn.cursor()
    c.execute('''
        INSERT INTO update_history (version, applied_by, status)
        VALUES (?, ?, 'completed')
    ''', (update_version, session['user_id']))
    conn.commit()
    conn.close()
...
def upgrade_database():
    ...
    # إضافة بيانات افتراضية للإعدادات
...
if __name__ == '__main__':
    ...
    print(f"🚀 OTP KING Dashboard - Owner: mohaymen")
    print(f"🌐 http://localhost:{port}")
    print(f"📁 Database: {DB_PATH}")
    print(f"👑 Owner login: mohaymen / mohaymen")
